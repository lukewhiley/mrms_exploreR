...
print(paste0("Succesfully imported ", project_name, ": ", nrow(mrms_exploreR_data$data_unprocessed), " samples; ", length(feature), " MRMS features"))
...
kable(tic_qc_fail,
      caption = paste0(nrow(tic_qc_fail), " samples FAILED the TIC QC check  ", nrow(tic_qc_fail_ltr),"  were QCs.  These have been removed from the dataset."))
...
# download and parse data preparation script
mrms_exploreR_qc_check_1 <- GET(url = paste0(parent_github_function_url, "/scripts/2021-LGW-mrms_exploreR_script.R")) %>% 
                               content(as = "text")
...
if(remove_qc == "keep"){mrms_exploreR_data$data_tic_filtered_qc_filtered <- mrms_exploreR_data$data_tic_filtered}
if(remove_qc == "remove"){mrms_exploreR_data$data_tic_filtered_qc_filtered <- mrms_exploreR_data$data_tic_filtered %>% select(sampleID, type, run_order, plateID, all_of(metabolite_list_filtered_2))}


QC_p_1
QC_p_2

paste0("QC check 1: ", length(metabolite_list_filtered), " had < 50 % zero values QC check.  ", length(failed_metabolites_zero), " had > 50% zero values.")
paste0("QC check 2: ", length(metabolite_list_filtered_2), " feature had an % RSD of < 30% in replicate QC samples.  ", length(failed_metabolites_rsd), " had an RSD > 30% in the PQC.")
...
lgw_pca(FUNC_individual_multivariate_data = mrms_exploreR_data$data_tic_filtered_qc_filtered,
        FUNC_metabolite_list = feature,
        FUNC_colour_by = "type",
...
signal_drift_correct_script <- GET(url = paste0(parent_github_url, "/scripts/2021-mrms-signal_driftR_script.r")) %>% 
                                     content(as = "text")
...
```{r, mrms_exploreR corrected signal QC check,  eval = FALSE, echo = FALSE, results = FALSE, warning=FALSE, message=FALSE}

# download and parse data preparation script
mrms_exploreR_qc_check_1 <- GET(url = paste0(parent_github_function_url, "/scripts/2021-LGW-mrmsd_exploreR_script.R")) %>% 
                               content(as = "text")
eval(parse(text = lipid_exploreR_script), envir = .GlobalEnv)
rm(lipid_exploreR_script)
